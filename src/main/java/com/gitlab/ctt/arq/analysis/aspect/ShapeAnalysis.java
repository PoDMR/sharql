...
public class ShapeAnalysis {
	...
	private final static Map<String, Function<DirectedGraph<Object, DefaultEdge>, Boolean>> shapeTests =
		...
		ImmutableMap.<String, Function<DirectedGraph<Object, DefaultEdge>, Boolean>>builder()
			...
			.put("singleNode", graph -> GraphShape.isSingeNode(graph))
			.put("noEdge", graph -> graph.edgeSet().size() == 0)
			.put("singleEdge", graph -> GraphShape.isSingeEdge(graph))
			.put("singleEdgeSet", graph -> GraphShape.isSingeEdgeSet(graph))
			.put("nonBranching", graph -> GraphShape.isNonBranching(graph))
			.put("nonBranchingSet", graph -> GraphShape.isNonBranchingSet(graph))
			.put("limitedBranching", graph -> GraphShape.isLimitedBranching(graph))
			.put("limitedBranchingSet", graph -> GraphShape.isLimitedBranchingSet(graph))
	...
	public static final int AFOSBD = 1|8|4|(1<<SERVICE-1)|(1<<BIND-1)|(1<<DATA-1);
	...
	public static int countVars(Element element) {
	...
	public static int countConstants(Element element) {
	...
	public static List<Integer> countValues(Element element) {
	...
	public static int optNest(Element element) {
		try {
			ElementDeepWalker.walk(element, new ElementVisitorBase() {
				@Override
