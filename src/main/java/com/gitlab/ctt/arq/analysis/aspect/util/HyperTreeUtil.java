...
public class HyperTreeUtil {
	...
	public static Map<String, Object> queryStrHtd(String queryStr,
			boolean skipPredicates, boolean skipConstants) {
		Either<Exception, Query> maybeQuery = SparqlUtil.get().toQuery(queryStr);
		if (maybeQuery.isRight()) {
			Query query = maybeQuery.right().value();
			DirectedGraph<String, String> graph = new DirectedPseudograph<>(String.class);
			Map<String, String> hgLabels = new LinkedHashMap<>();
			Map<String, String> gmlLabels = new LinkedHashMap<>();
			for (int k = 1; k < LIMIT; k++) {
				int state = detkdecomp(query, k, skipPredicates, skipConstants, file -> {
					if (file != null) {
						if (MARKER.getName().equals(file.getName())) {
							return HYPER_TREE_ERROR;
						}
						...
						try {
							gmlImporter.importGraph(graph, file);
							hgLabels.putAll(readLabels(file));
							return 0;
						} catch (ImportException e) {
							LOGGER.error("GML import exception", e);
						}
				...
				if (state != HYPER_TREE_ERROR) {
					return toJsonLike(graph, hgLabels, gmlLabels);
				}
			}
		}
		Map<String, Object> noResult = new LinkedHashMap<>();
		noResult.put("edges", Collections.emptyList());
		noResult.put("labels", Collections.emptyList());
		return noResult;
	}

	private static Map<String, String> readLabels(File gmlFile) {
		String pathname = gmlFile.getAbsoluteFile().toString().replaceAll(".gml$", ".yaml");
		File yamlFile = new File(pathname);
		Yaml yaml = new Yaml();
	...
	private static Map<String, Object> toJsonLike(DirectedGraph<String, String> graph,
			Map<String, String> hgLabels, Map<String, String> gmlLabels) {
		List<List<String>> pairs = new ArrayList<>();
		graph.vertexSet().forEach(v -> {
			Set<String> edges = graph.outgoingEdgesOf(v);
			edges.forEach(e -> {
				pairs.add(Arrays.asList(graph.getEdgeSource(e), graph.getEdgeTarget(e)));
			});
		});

		Map<String, String> labels = new LinkedHashMap<>();
		gmlLabels.forEach((k, v) -> {
			List<String> labelParts = new ArrayList<>();
			Matcher m = Pattern.compile("\\b\\d+").matcher(v);
			while (m.find()) {
				labelParts.add(hgLabels.get(v.substring(m.start(), m.end())));
			}
			labels.put(k, String.join("\n", labelParts));
		});
		Map<String, Object> result = new LinkedHashMap<>();
		result.put("edges", pairs);
